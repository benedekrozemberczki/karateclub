...
class SCD(Estimator):
    ...
    def _create_inverse_community_index(self):
        ...
        for node, cluster_membership in self._cluster_memberships.items():
            if cluster_membership in inverse_community_index:
                inverse_community_index[cluster_membership] = inverse_community_index[
                    cluster_membership
                ].union({node})
            else:
    ...
    def _calculate_community_statistics(self, inverse_community_index):
        ...
        for comm, members in inverse_community_index.items():
            ...
            edge_out = sum(
                [
                    0 if neighbor in induced_graph else 1
                    for node in members
    ...
    def _calculate_theta_2(self, r, d, b, q):
        ...
        theta_2 = -(theta_2_left_enum / theta_2_left_denom) * (
            theta_2_right_enum / theta_2_right_denom
        )
        return theta_2
    ...
    def _calculate_theta_3(self, r, d, b, q, d_out, d_in):
        ...
        theta_3_left_denom = (
            theta_3_left_enum
            + d_out * (d_out - 1) * self._omega
            + d_out * d_in * self._omega
            + self.eps
        )
        theta_3_right_enum = d_in + d_out
        theta_3_right_denom = r + d_out + self.eps
        theta_3 = (theta_3_left_enum / theta_3_left_denom) * (
            theta_3_right_enum / theta_3_right_denom
        )
        return theta_3
    ...
    def _do_refinement(self):
        ...
        community_statistics = self._calculate_community_statistics(
            inverse_community_index
        )
        community_index = self._find_community_index(community_statistics)
        for node in self._nodes:
            community_level_stats = community_statistics[
                self._cluster_memberships[node]
            ]
            neighbors = set([neighbor for neighbor in self._graph.neighbors(node)])
            candidate_communities = [
                self._cluster_memberships[neighbor] for neighbor in neighbors
            ]
            d_out = len(
                set(neighbors).difference(
                    inverse_community_index[self._cluster_memberships[node]]
                )
            )
            ...
            for comm in candidate_communities:
                ...
                d_in = len(neighbors) - d_out
                WCC_aux = (
                    self._calculate_wcc(community_level_stats, d_out, d_in) + WCC_r
                )
                if WCC_aux > WCC_t:
                    WCC_t = WCC_aux
                    best_community = comm
